...
class TestFeatureDefaultModels:
    def test_high_judgment_surfaces_default_to_luna(self) -> None:
        ...
        # lead three cases were too few to confirm.
        assert DEFAULT_INSIGHTS_MODEL == OPENAI_LUNA_MODEL
        # Coach followed on 2026-08-09, but unmeasured: the harness has no
        # coach runner, so nothing ever showed Opus 5 was worth 23x here.
        assert DEFAULT_COACH_MODEL == OPENAI_LUNA_MODEL
...
class TestPromptRenderers:
    ...
    def test_uncounted_short_lift_is_labelled_in_the_day_card(self) -> None:
        """The card must not show a lift the week total silently leaves out."""
        data = self._health_data()
        data["current_week"]["days"][2]["workouts"] = [
            {
        ...
        rendered = render_health_data(data, prompt_kind="nudge", today=date(2026, 4, 8))

        assert (
            "Functional Strength Training (9 min; short session, not counted "
            "as a lift)" in rendered
        )
        assert "Traditional Strength Training (51 min)" in rendered

    def test_day_card_sleep_is_labelled_as_the_night_before(self) -> None:
        rendered = render_health_data(
            self._health_data(), prompt_kind="nudge", today=date(2026, 4, 8)
        )

        assert "- Sleep (night before): pending sync." in rendered
        assert "- Sleep (night before): 8.1 h" in rendered
        assert "- Sleep: " not in rendered.split("### Today", 1)[1]

    def test_unavailable_hrv_trend_is_omitted(self) -> None:
        rendered = render_health_data(
            self._health_data(), prompt_kind="nudge", today=date(2026, 4, 8)
        )

        assert "HRV trend" not in rendered
...
class TestFormatLastCoachSummary:
    def test_recent_review_is_shown_with_its_date(self) -> None:
        result = format_last_coach_summary(
            "Keep tempo conditional.", "2026-09-14", today=date(2026, 9, 21)
        )

        assert result == "[2026-09-14] Keep tempo conditional."

    def test_stale_review_is_withheld(self) -> None:
        """An April review must not read as September's coaching touchpoint."""
        result = format_last_coach_summary(
            "The tempo run proved you have the legs.",
            "2026-04-07",
            today=date(2026, 9, 23),
        )

        assert "tempo" not in result
        assert result.startswith("(no coach review in the last")

    def test_review_on_the_age_limit_is_still_shown(self) -> None:
        today = date(2026, 9, 23)
        written = today - timedelta(days=COACH_SUMMARY_MAX_AGE_DAYS)

        result = format_last_coach_summary("Summary.", written.isoformat(), today=today)

        assert result.endswith("Summary.")

    def test_missing_or_undated_review(self) -> None:
        assert format_last_coach_summary("", "") == "(no recent coach review)"
        assert (
            format_last_coach_summary("Summary.", "", today=date(2026, 9, 23))
            == "(no recent coach review)"
        )
...
class TestBuildLlmData:
    ...
    def test_history_starts_on_a_monday(
        self, mock_date: MagicMock, in_memory_db: sqlite3.Connection
    ) -> None:
        """A window cut mid-week must not render a two-day week as a real one."""
        ...
        store_snapshots(
            in_memory_db,
            [
                DailySnapshot(date=(start + timedelta(days=i)).isoformat(), steps=5000)
                for i in range(70)
            ],
        )

        result = build_llm_data(in_memory_db, months=1)

        # 30 days before Monday 9 Mar is Saturday 7 Feb; history begins Monday 9 Feb.
        first_label = result["history"][0]["summary"]["week_label"]
        assert "2026-02-09" in first_label
    ...
    def test_manual_sleep_overrides_imported_sleep_in_prompt_context(
        ...
        assert "Sleep (night before): 7.0 h" in rendered
