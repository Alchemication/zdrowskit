...
Mermaid diagrams in `docs/*.md` are pre-rendered to SVG by `marketing/render_diagrams.py` (needs Chrome) and committed under `marketing/site/assets/diagrams/`, keyed by a hash of the diagram source. The build inlines them and fails if a block has no matching SVG, so re-run the renderer after editing any ```mermaid block. The published pages ship no Mermaid runtime; the Markdown keeps the source so GitHub still renders it.