...
# A delivered nudge opens with a bolded trigger header, optionally followed by
# a middot and one line of measured weekly progress. Neither is something the
# model wrote, so neither belongs in the digest of what the user was already
# told — replaying them teaches the next nudge to imitate its own chrome. The
# shape is matched rather than the header names, which change with the trigger
# vocabulary; a whole-line bold in a nudge body is chrome here by convention.
_NUDGE_HEADING_RE = re.compile(r"\*\*[^*]+\*\*(?:\s*·\s*.*)?")
...
def _clean_nudge_text(text: str) -> str:
    ...
    for raw_line in text.splitlines():
        ...
        if line.startswith("_Generated by "):
            continue
        if _NUDGE_HEADING_RE.fullmatch(line):
            continue
...
def build_llm_data(
    ...
    Returns:
        A dict with 'current_week', 'history', 'since_days', 'week_complete',
        'week_label', 'week_start' and 'week_end' keys, JSON-serialisable. The
        early return for an empty database carries only 'current_week',
        'history' and 'week_complete'.
    """
