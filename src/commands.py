...
def cmd_coach(
    ...
    # If the loop exited with an empty response (iteration cap reached while
    # the model still wanted tools, or model returned empty text), force one
    # final tool-less synthesis call so we never ship a blank coach review.
    if not (result.text or "").strip():
        logger.warning(
            "Coach loop exited with empty text (tool_calls=%s); forcing final synthesis",
            bool(result.tool_calls),
        )
        if result.tool_calls:
            messages.append(result.raw_message)
