...
def cmd_nudge(
    ...
    # SKIP is a valid empty-ish answer, but only when the model produced no
    # pending tool calls AND voluntarily wrote nothing — which is the natural
    # exit through the `not result.tool_calls` branch above. An empty result
    # *with* pending tool_calls means we cut the model off mid-investigation.
...
def cmd_coach(
    ...
    # tools, force one final tool-less synthesis call so we never strand a
    # half-finished review. An empty text with NO pending tool calls is a
    # voluntary SKIP — handled below — so we do not force synthesis there.
    if not (result.text or "").strip() and result.tool_calls:
        logger.warning(
    ...
    # noise.
    #
    # Only honor SKIP when there are also no pending edits — a model that
    # proposed update_context calls AND returned SKIP is contradicting
    # itself, in which case we fall through and let the regular path send
    # the (likely truncated) reasoning so the edits aren't silently dropped.
