...
def cmd_coach(args: argparse.Namespace) -> tuple[str, list[ContextEdit]]:
    ...
    concrete edits via ``<context_update>`` blocks. Proposals are returned
    ...
    edits = extract_all_context_updates(result.text)
    visible_text = strip_all_context_updates(result.text).strip()
    visible_text += f"\n\n---\n_Generated by {result.model}_"
