...
- **Verifier introduced the bug**: model-quality issue. A/B verifier route reasoning via `main.py models` or Telegram `/models` (DeepSeek `high` engages thinking; `medium` leaves it off). Capture as `nudge_verify` real_regression if reproducible.
...
When stage and surface are clear, switch to `llm-evals`. Supported eval features today: `chat` (full tool loop, `--model`-driven) and `nudge_verify` (verifier-only, env-driven).
