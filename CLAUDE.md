...
DeepSeek V4 models, including Pro and Flash, default to thinking mode enabled with high effort unless explicitly disabled. Verifier calls intentionally pass `extra_body={"thinking": {"type": "disabled"}}` and `response_format={"type": "json_object"}` via `src/llm_verify.py`; keep provider-specific params model-aware in `src/llm.py` so Anthropic fallbacks omit DeepSeek-only fields. Draft/rewrite calls do not globally disable DeepSeek thinking unless config/call sites explicitly request it.
